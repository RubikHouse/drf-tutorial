...
def snippet_list(request):
    if request.method == 'GET':
        ...
        # Snippet QuerySet을 생성자로 사용한 SnippetSerializer 인스턴스
        serializer = SnippetSerializer(snippets, many=True)
        # JSON형식의 문자열을 HttpResponse로 돌려줌 (content_type에 'application/json'명시됨)
        return JsonResponse(serializer.data, safe=False)

    elif request.method == 'POST':
        # request를 분석해서 전달받은 JSON형식의 문자열을 파이썬 데이터형으로 파싱
        data = JSONParser().parse(request)
        # data인수를 채우면서 Serializer인스턴스 생성 (역직렬화)
        serializer = SnippetSerializer(data=data)
        # Serializer의 validation
        if serializer.is_valid():
            # valid한 경우, Serializer의 save()메소드로 새 Snippet인스턴스 생성
            serializer.save()
